...
class Tauler(object):
    ...
    def __init__(self, mida, vaixells=[], nomTauler='', vides=0, lang='ca'):
        ...
        #Poblar la matriu:
        matriuPoblada = False
        while not(matriuPoblada):
            try:
                self.colocarVaixells()
                matriuPoblada = True
            except:
                self.matriu = self.iniciarTauler(self.mida)
                print("No s'ha pogut poblar el tauler, reintentant...")
        ...
        for vaixell in self.llVaixells:
            ...
            for q in range(vaixell['quantitat']):
                ...
                while not(vaixellColocat):
                    if tries > 50:
                        raise Exception("No s'han pogut colocar els vaixells al tauler.")
                    tries += 1
    ...
    def tret(self, coords):
        if self.jocAcabat:
            return get.text('playground/winMessage', self.lang)
            
        if self.videsActivades and (self.getVides() == 0):
            return get.text('playground/deathMessage', self.lang)
        ...
        if not(casella.casellaOberta()):
            if casella.teVaixell():
                ...
                if vaixellMort:
                    ...
                    return get.text('playground/sink', self.lang, vaixell.getName().upper())
                else:
                    return get.text('playground/touched', self.lang)
            else:
                self.restarVida()
                return get.text('playground/water', self.lang)
